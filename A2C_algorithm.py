...
class A2C_pok(RlAlgorithm):
    """
    Inherits from RlAlgorithm class from rlypt library is constructed to trains 
    agents using samples from the buffer and performing gradient descent on the
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        if itr % self.train_every != 0:
            return opt_info
        for i in tqdm(range(self.train_steps), desc="A2C"):
        ...
        with torch.no_grad():
            if self.use_pcont:
                ...
                flat_post = buffer_method(
                    prior[:-1, :], "reshape", (batch_t - 1) * (batch_b), -1
                )
            else:
                flat_post = buffer_method(prior, "reshape", batch_size, -1)
                #print('POST SIZE:', post.deter.size())
                #print('FLAT POST SIZE:', flat_post.deter.size())
